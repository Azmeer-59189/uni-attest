...
import { Upload, ArrowLeft, CheckCircle, Clock, FileText, AlertCircle, X } from 'lucide-react'
...
const NewApplication = () => {
  ...
  // On mount — check if student already has an active application
  useEffect(() => {
  ...
  // ── Existing active application found ──
  if (existingApp) {
    const cfg = STATUS_CONFIG[existingApp.status] || STATUS_CONFIG.pending
    const Icon = cfg.icon
    const canResubmit = existingApp.status === 'rejected'
        ...
        <div className="flex items-center mb-8">
          {['Details', 'Documents', 'Done'].map((label, i) => (
            <div key={label} className="flex items-center">
              ...
              <span className={`ml-2 text-sm ${step === i + 1 ? 'text-navy font-medium' : 'text-gray-400'}`}>
                {label}
              </span>
